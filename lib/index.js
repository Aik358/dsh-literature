...
var CONFIG_PATH, STORE_DIR, PDF_DIR, STORE_PATH, DEFAULTS, cached;
var init_config = __esm({
  "src/node/config.js"() {
    init_log();
    CONFIG_PATH = join(dshHome(), "dsh-literature.json");
    STORE_DIR = join(dshHome(), "storages", "dsh-literature");
    PDF_DIR = join(STORE_DIR, "pdfs");
    STORE_PATH = join(STORE_DIR, "store.json");
    DEFAULTS = {
      version: 1,
...
async function getImportedFile(absPath) {
  return (await load()).importedFiles[absPath] ?? null;
}
async function addImportedFile(absPath, mtimeMs) {
  const s = await load();
...
var EMPTY, state, flushTimer, flushing;
var init_db = __esm({
  "src/node/store/db.js"() {
    init_config();
    init_log();
    EMPTY = { version: 1, items: {}, tasks: {}, annotations: {}, importedFiles: {} };
    state = null;
    flushTimer = null;
    flushing = null;
  }
...
var init_net = __esm({
  "src/node/net.js"() {
    init_log();
    UA = "dsh-literature/0.1 (+https://github.com/deepseek-ai/deepseek-harness)";
...
var BASE_FIELDS, TYPE_FIELDS;
var init_normalize = __esm({
  "src/node/metadata/normalize.js"() {
    BASE_FIELDS = ["title", "abstractNote", "shortTitle", "url", "accessDate", "language", "rights", "extra", "DOI"];
    TYPE_FIELDS = {
...
var PDF_MAGIC, PdfFailure, LOGIN_HINT;
var init_pdf = __esm({
  "src/node/fetch/pdf.js"() {
    init_net();
    init_log();
    PDF_MAGIC = Buffer.from("%PDF-", "latin1");
    PdfFailure = class extends Error {
    ...
    LOGIN_HINT = /\bsign\s*in\b|\blog\s*in\b|login|登录|登錄|institutional\s*access|sso|okta|ezproxy|shibboleth|athens|your\s+institution/i;
  }
});
...
async function getItemChildren(key) {
  ...
  try {
    ...
    if (wantMode === "dir") {
      ...
      if (current.pdf?.path) {
        ...
        buffer = await readFile5(current.pdf.path).catch(() => null);
      }
    ...
    if (current.pdf?.path) {
      const { readFile: readFile5 } = await import("node:fs/promises");
      pdfBuffer = await readFile5(current.pdf.path).catch(() => null);
    }
    if (!pdfBuffer) {
      const fetched = await fetchItemPdf(key);
      if (fetched?.pdf?.path) {
        const { readFile: readFile5 } = await import("node:fs/promises");
        pdfBuffer = await readFile5(fetched.pdf.path).catch(() => null);
      }
    }
...
function apa(record) {
  const authors2 = authorsList(record, { max: 20 });
  const who = authors2 ? `${authors2}. ` : "";
...
function gb(record) {
  ...
  if (record.itemType === "book" || record.itemType === "report" || record.itemType === "thesis") {
    const tag2 = record.itemType === "thesis" ? "D" : record.itemType === "report" ? "R" : "M";
    const pub = record.publisher ? `${record.publisher}, ` : "";
    return `${whoPart}${title}[${tag2}]. ${pub}${year}.`.trim();
  }
  if (record.itemType === "conferencePaper") {
...
function safePdfPath(key) {
  const base = resolve5(PDF_DIR);
  const candidate = resolve5(base, `${String(key).replace(/[^\w.-]+/g, "_")}.pdf`);
...
async function handler(req, res) {
  ...
  try {
    ...
    if (head === "cite" && methodOk(req, "POST")) {
      const body = await readJsonBody2(req);
      const item = await getItem(String(body?.key ?? ""));
      if (!item?.record) {
        writeJson(res, 404, { error: "\u6761\u76EE\u7F3A\u5C11\u5143\u6570\u636E\uFF0C\u65E0\u6CD5\u751F\u6210\u5F15\u7528" });
        return;
      }
      const { cite: cite2 } = await Promise.resolve().then(() => (init_cite(), cite_exports));
      const text = cite2(item.record, {
    ...
    if (head === "scan-dir" && methodOk(req, "POST")) {
      const body = await readJsonBody2(req);
      const { importDir: importDir2 } = await Promise.resolve().then(() => (init_importer(), importer_exports));
      const config = await loadConfig();
      const result = await importDir2(body?.dir || config.importDir, { autoResolve: body?.autoResolve !== false });
    ...
    if (head === "import-zotero" && methodOk(req, "POST")) {
      ...
      const result = await importFromZotero2({ limit: Number(body?.limit ?? 50) });
      writeJson(res, 200, result);
      return;
    }
...
function apply(ctx, config) {
  ...
  const ready = (async () => {
    ...
    try {
      const { startWatcher: startWatcher2 } = await Promise.resolve().then(() => (init_importer(), importer_exports));
      disposers.push(startWatcher2());
    } catch (e) {
